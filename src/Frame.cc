...
mpReferenceKF(static_cast<KeyFrame*>(nullptr)), mbIsSet(false), mbImuPreintegrated(false), mbHasPose(false),
mbHasVelocity(false)
{
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp,
             ...
             const IMU::Calib &ImuCalib,const cv::Mat &mask)
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc,
    ...
    :mpcpi(nullptr),mpORBvocabulary(voc),mpORBextractorLeft(extractor),
    mpORBextractorRight(static_cast<ORBextractor*>(nullptr)),
     mTimeStamp(timeStamp), mK(dynamic_cast<Pinhole*>(pCamera)->toK()), mK_(dynamic_cast<Pinhole*>(pCamera)->toK_()),
     mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(nullptr),mpPrevFrame(pPrevF),mpImuPreintegratedFrame(nullptr),
     mpReferenceKF(static_cast<KeyFrame*>(nullptr)), mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera),
...
void Frame::UndistortKeyPoints()
{
    ...
    // Undistort points
    mat=mat.reshape(2);
    cv::undistortPoints(mat,mat, dynamic_cast<Pinhole*>(mpCamera)->toK(),mDistCoef,
                        cv::Mat(),mK);
    mat=mat.reshape(1);
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)
{
    if(mDistCoef.at<float>(0)!=0.0)
    {
        ...
        mat=mat.reshape(2);
        cv::undistortPoints(mat,mat,dynamic_cast<Pinhole*>(mpCamera)->toK(),
                            mDistCoef,cv::Mat(),mK);
        mat=mat.reshape(1);
...

{
    ...
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,dynamic_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[0],
                      dynamic_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[1],cv::Mat());
    thread threadRight(&Frame::ExtractORB,this,1,imRight,dynamic_cast<KannalaBrandt8*>(mpCamera2)->mvLappingArea[0],
                       dynamic_cast<KannalaBrandt8*>(mpCamera2)->mvLappingArea[1],cv::Mat());
    threadLeft.join();
    threadRight.join();
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
...
void Frame::ComputeStereoFishEyeMatches() {
    ...
    for(auto & matche : matches){
        if(matche.size() >= 2 && matche[0].distance < matche[1].distance * 0.7){
            ...
            Eigen::Vector3f p3D;
            descMatches++;
            float sigma1 = mvLevelSigma2[mvKeys[matche[0].queryIdx + monoLeft].octave],
            sigma2 = mvLevelSigma2[mvKeysRight[matche[0].trainIdx + monoRight].octave];
            float depth = dynamic_cast<KannalaBrandt8*>(mpCamera)->TriangulateMatches(
                    mpCamera2,mvKeys[matche[0].queryIdx + monoLeft],mvKeysRight[matche[0].trainIdx
                    + monoRight],mRlr,mtlr,sigma1,sigma2,p3D);
            if(depth > 0.0001f){
