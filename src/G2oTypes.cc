...
{
    const VertexPose* VP1 = dynamic_cast<const VertexPose*>(_vertices[0]);
    const VertexVelocity* VV1= dynamic_cast<const VertexVelocity*>(_vertices[1]);
    const VertexGyroBias* VG1= dynamic_cast<const VertexGyroBias*>(_vertices[2]);
    const VertexAccBias* VA1= dynamic_cast<const VertexAccBias*>(_vertices[3]);
    const VertexPose* VP2 = dynamic_cast<const VertexPose*>(_vertices[4]);
    const VertexVelocity* VV2= dynamic_cast<const VertexVelocity*>(_vertices[5]);
    const IMU::Bias b1(VA1->estimate()[0],VA1->estimate()[1],VA1->estimate()[2],
                       VG1->estimate()[0],VG1->estimate()[1],VG1->estimate()[2]);
...
void EdgeInertialGS::computeError()
{
    ...
    const Eigen::Vector3d ep = VP1->estimate().Rwb.transpose()*(s*(VP2->estimate().twb - VP1->estimate().twb
            - VV1->estimate()*dt) - g*dt*dt/2) - dP;
...
void EdgeInertialGS::linearizeOplus()
{
    ...
    _jacobianOplus[0].block<3,3>(6,0) =
            Sophus::SO3d::hat(Rbw1*(s*(VP2->estimate().twb - VP1->estimate().twb
