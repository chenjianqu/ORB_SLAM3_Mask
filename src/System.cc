    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    "Montiel and Juan D. Tardós, University of Zaragoza." << endl <<
    ...
    if(!fsSettings["System.IsMask"].empty()){
        fsSettings["System.IsMask"] >> mbMask;
        cout<<"System.IsMask:"<<mbMask;
    }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp,
                               const vector<IMU::Point>& vImuMeas, string filename,const cv::Mat &mask)
{
