...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp,
                                     string filename,const cv::Mat &mask)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,
                              mK,mDistCoef,mbf,mThDepth,mpCamera,static_cast<Frame*>(nullptr),
                              IMU::Calib(),mask);
...
bool Tracking::PredictStateIMU()
{
    ...
    if(mbMapUpdated && mpLastKeyFrame)
    {
        ...
        Eigen::Matrix3f Rwb2 = IMU::NormalizeRotation(Rwb1 *
                mpImuPreintegratedFromLastKF->GetDeltaRotation(mpLastKeyFrame->GetImuBias()));
        ...
        Eigen::Vector3f Vwb2 = Vwb1 + t12*Gz + Rwb1
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        && (mCurrentFrame.mnId > mnFramesToResetIMU) &&
           ...
           || mSensor == System::IMU_RGBD) && pCurrentMap->isImuInitialized())
        {
...
void Tracking::MonocularInitialization()
{
    ...
    {
        if (((int)mCurrentFrame.mvKeys.size()<=100)||((mSensor == System::IMU_MONOCULAR)
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR)
    {
        ...
        // non tracked closed points: " + to_string(nNonTrackedClose), Verbose::VERBOSITY_NORMAL);
    ...
    bool c4 = false;
    // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15))
