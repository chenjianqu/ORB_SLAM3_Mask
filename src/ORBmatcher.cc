...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints,
                                       const float th, const bool bFarPoints, const float thFarPoints) const
    {
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched,
                                            vector<int> &vnMatches12, int windowSize) const
    {
        int nmatches=0;
                ...
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    for(unsigned long idx2 : f2it->second)
                    {
                        ...
                        if(bCoarse || pCamera1->epipolarConstrain(
                                ...
                                pKF1->mvLevelSigma2[kp1.octave],pKF2->mvLevelSigma2[kp2.octave])) // MODIFICATION_2
                        {
                            bestIdx2 = idx2;
                            bestDist = dist;
                        }
